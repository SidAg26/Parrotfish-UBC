...
function_name = 'workbench-chameleon'
json_file_path = '/home/ubuntu/memFigLessDIR/Parrotfish-UBC/results-workbench-chameleon.json'
...
# Parse the JSON data
parsed_data = sorted(parsed_data, key=lambda x: x['payload']['num_of_rows'])
# parsed_data = [x for x in parsed_data if 'init_duration' in x and x['init_duration'] is None]
payload = []
memory = []

for item in parsed_data:
    payload.append(item['payload']['num_of_rows'])
    ...
    if match:
        ...
        print('No match found for ipnut:', item['payload']['num_of_rows'])
        result = 0
...
for i in range(len(payload)):
    if memory[i] != 0:
        ...
        if _invoke_result:
            ...
            with open('chameleon_data.json', 'a') as file:
