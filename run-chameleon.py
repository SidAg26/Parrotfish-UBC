...
config_file_path = './config-chameleon.json'
# Path to the results file
results_file_path = './results-workbench-chameleon.json'
# Payload file path 
payload_file_path = '/home/ubuntu/memFigLessDIR/Mem-Fig-Less/estimation-algorithms/results-workbench-chameleon.json'
...
payload_list = [{'num_of_rows': item['payload'], 'num_of_cols': 25} for item in _payload if 'payload' in item]
# _duration = [item['billed_duration'] for item in _payload]
# mean_duration = int((sum(_duration) / len(_duration))*1.1)
mean_duration = int(2006 * 1.5) # 50% increase in the mean duration from metadata
