...
config_file_path = './config-dynamic-html.json'
# Path to the results file
results_file_path = './results-dynamic-html.json'
payload_file_path = '/home/ubuntu/memFigLessDIR/Mem-Fig-Less/estimation-algorithms/results-dynamic-html.json'
...
payload_list = [{'n': int(item['payload']) if type(item['payload']) is not int else item['payload'] } for item in _payload if 'billed_duration' in item]
# _duration = [item['billed_duration'] for item in _payload if 'billed_duration' in item]
# mean_duration = int((sum(_duration) / len(_duration))*1.1)
mean_duration = int(76 * 1.5) # 50% increase in the mean duration from metadata
...
# # Path to the results file
# results_file_path = './results-graph-mst-2.json'
# # Payload file path 
# payload_file_path = '/home/ubuntu/memFigLessDIR/Mem-Fig-Less/estimation-algorithms/results-graph-mst.json'
...
# _duration = [item['billed_duration'] for item in _payload if 'billed_duration' in item]
# # mean_duration = int((sum(_duration) / len(_duration))*1.1)
# mean_duration = int(4646 * 1.5) # 50% increase in the mean duration from metadata
