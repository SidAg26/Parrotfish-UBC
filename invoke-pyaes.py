...
function_name = 'workbench-pyaes'
json_file_path = '/home/ubuntu/memFigLessDIR/Parrotfish-UBC/results-workbench-pyaes.json'
...
# Parse the JSON data
parsed_data = sorted(parsed_data, key=lambda x: x['payload']['length_of_message'])
# parsed_data = [x for x in parsed_data if 'init_duration' in x and x['init_duration'] is None]
payload = []
memory = []

for item in parsed_data:
    payload.append(item['payload']['length_of_message'])
    ...
    if match:
        ...
        print('No match found for ipnut:', item['payload']['length_of_message'])
        result = 0
...
for i in range(len(payload)):
    if memory[i] != 0:
        ...
        if _invoke_result:
            ...
            with open('pyaes_data.json', 'a') as file:
