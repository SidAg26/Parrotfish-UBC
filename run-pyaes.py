...
config_file_path = './config-pyaes.json'
# Path to the results file
results_file_path = './results-workbench-pyaes.json'
# Payload file path 
payload_file_path = '/home/ubuntu/memFigLessDIR/Mem-Fig-Less/estimation-algorithms/results-workbench-pyaes.json'
...
payload_list = [{"length_of_message": item['payload'], "num_of_iterations": 10} for item in _payload if 'payload' in item]
# _duration = [item['billed_duration'] for item in _payload]
# mean_duration = int((sum(_duration) / len(_duration))*1.1)
mean_duration = int(1392 * 1.5) # 50% increase in the mean duration from metadata
